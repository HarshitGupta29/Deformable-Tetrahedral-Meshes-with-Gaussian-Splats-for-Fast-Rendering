...
##################################################################
def generate_edge(tet_list_tx4, faster = False):
    r"""
    Agrs:
    ...
    if faster:
        edges_all_ex2, indices = np.unique(edges_all_ex2, axis=0, return_inverse=True)
        return edges_all_ex2, indices.reshape((-1, 6),order='F')
    else:
        return np.unique(edges_all_ex2, axis=0)
...
def generate_subdivision(tet_list_tx4,
    ...
    so = time.time()
    # 1) get edge
    edges_all_ex2, tet_list_edge_idx_tx6 = generate_edge(tet_list_tx4, faster=True)
