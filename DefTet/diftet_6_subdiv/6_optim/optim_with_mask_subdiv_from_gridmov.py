...
def optimzie(images,
    ...
    #deftetmodel.tfpoint_px3.requires_grad_()
    #print(len(list(deftetmodel.parameters())))
    #input()
    ...
    if loadpath is not None:
        loadpath = '/u/singularity/DefTet/final_output/hotdog-tet40-dim4_0cham/stage-sub_2-carve_0/deftet.pth'
    ...
    v = deftetmodel.get_vol().cpu().detach().numpy()
    quantile_value = np.quantile(v, 0.05)

# Count the numbers in the tensor that are less than or equal to the quantile value
    ri = np.sum(v <= quantile_value)
    print(ri)
    input()
    ...
    if True:
        for i in range(optnum):
            ...
            #centeroids = deftetmodel.points_px3[deftetmodel.tet_list_tx4].sum(axis=1)
            #print('SHAPE',deftetmodel.tet_list_tx4.shape, centeroids.shape)
            #input()
            torch.cuda.synchronize()
            ...
            #print(deftetmodel.tfpoint_px3)
            #input()
            #loss=torch.Tensor(0.)
            loss = loss0 * w_im + loss1 * w_mask + loss_occ * w_occ
            #loss = loss_occ
            if gaussians and not gridmov:
                mean, whitening = gaussians
                #mean = 0.5*torch.nn.functional.normalize(mean)+0.5
                #whitening = whitening.detach()
                N, _ = mean.shape
                points = deftetmodel.get_point()
                centroids = 0.25 * points[deftetmodel.tftet_tx4].sum(axis=1)
                #whitening = torch.nn.functional.normalize(whitening)
                so = time.time()
                c, _ = chamfer_distance(mean[None,:,:], centroids[None,:,:])
                dist1, dist2, idx1, idx2 = cham3D(mean[None,:,:], centroids[None,:,:], whitening[None, :, :])

                #print(mean[0], idx1[0,0], whitening[0], centroids[0:10])
                #input()
                #a = torch.mean(dist1)
                #a1 = torch.mean(dist2)
                #print(a1)
                ...
                k = torch.mean(dist2) + torch.mean(dist1)
                #print(k, loss)
                loss += k
                #print(loss)
                chamfer_time += time.time()-so
    ...
    print("chamfer_time: ", chamfer_time)
...
if __name__ == '__main__':
    ...
    if args.gaussianpth != '':
        ...
        gaussians.load_ply(args.gaussianpth)
        mean = gaussians.get_xyz #[N, 3]
        whitening = gaussians.get_covariance() #[N, 6]
        #print(mean.device, whitening.device)
        #input()
        #whitening = batch_whitening(cov)
        gaussians = (mean.detach(), whitening.detach())
    ...
    ###############################################################
    svfolder = '%s/%s-tet%d-dim%d_%d%s' % (args.savedir, expname, tetres, tetdim,
                                         tetdim_fixed, cham)
    ...
    for i in range(sublevel):
        for gridmov in [True, False]: #[False, True]:
            ...
            optimzie(images,
                     ...
                     dev='cuda',
                     gridmov=gridmov,
                     loadpath='/u/singularity/DefTet/final_output/drums-tet40-dim4_0/stage-sub_2-fix_0/deftet.pth',
                     sublevel=i,
                     lossweights=weights)
